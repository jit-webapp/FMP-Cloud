    ...
    async function saveToCloud(storeName, item) {
        if (window.auth && window.auth.currentUser && window.db) {
            try {
                ...
                let docId = item.id || item.key || item.name || item.type;
        ...
        try {
            ...
            for (const storeName of collectionsToSync) {
                ...
                if (!snapshot.empty) {
                    ...
                    // ให้โหลดจาก Cloud มาทับเครื่องนี้
    ...
    async function loadStateFromDB() {
		...
		async function initApp() {
			try {
				...
				if (state.password) {
					// ============================================================
					// [แก้ไขใหม่] : ตรวจสอบว่ารหัสผ่านเป็น Default หรือไม่ เพื่อเปลี่ยนข้อความแจ้งเตือน
					// ============================================================
					const hashedDefault = CryptoJS.SHA256(DEFAULT_PASSWORD).toString(); // Hash ของ '1234'
					
					// อ้างอิง Element ข้อความในหน้า Lock Screen (ใช้ querySelector เพื่อไม่ต้องแก้ HTML)
					const lockTitle = lockScreen.querySelector('h2'); 
					const lockDesc = lockScreen.querySelector('p');

					if (state.password === hashedDefault) {
						// === กรณีที่ 1: เป็นรหัสเริ่มต้น (1234) ===
						lockTitle.innerHTML = 'กรุณาใส่รหัสผ่าน';
						lockDesc.innerHTML = 'โปรดใส่รหัสผ่านเริ่มต้นเพื่อเข้าใช้งาน';
					} else {
						// === กรณีที่ 2: เปลี่ยนรหัสแล้ว (Custom Password) ===
						lockTitle.innerHTML = 'กรุณาใส่รหัสผ่าน';
